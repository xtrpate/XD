...
class Designer:
    ...
    def design(self):
        ...
        for index in range(len(code)):
            # tutorials on youtube mention `python3 dashboard.py` added the below check to keep them valid
            if (index == 0):
                self.output_path.joinpath("dashboard.py").write_text(code[index], encoding='UTF-8')
            else:
